...
export const Console=({
  ...
  function resetPrompt() {
    ...
    if (feedback) {
      interviewerPrompt +=
        " Provide constructive feedback on the candidate's answers, offer suggestions for improvement."
    } else {
      interviewerPrompt +=
        "Stay focused on a realistic counselling session simulation.";
    }
    ...
    if (enabledPersonalities.length > 0) {
      ...
      interviewerPersonality =
        "Warm, friendly, and encouraging. You are a supportive and helpful mental health counsellor.";
    }
    ...
    if (jobDescription.length == 0) {
      jobDescription =
        "Blank, kindly remind the user to follow complete instructions and then ask the user for a problems they are going in life";
    }

    const prompt = [
      {
        role: "system",
        content: `${interviewerPrompt} ${interviewerPersonality} ${interviewerQuestions} Limit your responses to 3 sentences. Do not respond with lists or ask multiple questions at once. End every response with a question to keep the conversation going. You are counselling a patient for the following problems: ${jobDescription}.`,
      },
    ];
              ...
              return (
                <div className={styles.Message} key={index}>
                  <span className={styles.Text}>
                    ...
                    <ul>
                      <li>
                        1. Choose your session settings (voice,
            ...
            if (message.role === "assistant") {
              return (
                <div className={styles.Message} key={index}>
                  <span className={styles.Text}>
                    {message.loading ? (
                      <i>Counsellor's response is being generated.</i>
                    ) : (
                      <>Coounsellor:&nbsp;{message.content}</>
                    )}
                  </span>
                  {message.audio && (
                    <>
